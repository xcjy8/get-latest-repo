...
```bash
sh /Users/sy/local-dev-workbench/sp-pro-ai/get-latest-repo/scripts/package.sh
```

项目只保留这一种本机打包入口。脚本会构建最新 release 二进制，安装到 `/Users/sy/local-bin/custom-getlatestrepo`，创建 `getrep` 命令入口，并修正 `/Users/sy/.binquick` 中旧的 `getrep` alias。当前终端如果已经加载过旧 alias，脚本结束后按提示执行一次：

```bash
source /Users/sy/.binquick
```
