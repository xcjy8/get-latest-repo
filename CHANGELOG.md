...
> Docker 生产部署、Git 获取代理修复、动态并发与大规模仓库性能优化

### Added

- **Docker 生产运行链路**：新增前后端多阶段镜像、Compose 编排、Nginx 反向代理、健康检查、只读根文件系统、非 root 运行、持久卷供给及卷生命周期验证脚本。
- **资源自适应并发**：按照容器实际 CPU 与内存配额动态计算 Fetch 和文件系统并发，并在 Web 配置面板展示最终生效值。

### Fixed

- **Docker 7890 代理修复**：容器统一通过 `host.docker.internal:7890` 访问宿主机代理，并以命令级 Git 配置覆盖仓库内错误的 `127.0.0.1:7890`。
- **批量获取失败风暴修复**：批量操作前预检代理端口，不可达时立即停止，避免数百个仓库分别超时和重复重试。
- **本地修改误报修复**：过滤未跟踪 Finder 元数据，并严格校验 Git LFS 水合文件，保留真实修改检测能力。
- **获取完成尾部卡顿修复**：Fetch 后仅刷新远程跟踪状态，不再重复遍历整个工作区；安全检查、扫描、获取复检和最终核对均提供真实阶段进度。
- **容器 Git 身份修复**：stash 与原生 Git 子进程使用进程级临时身份，不修改仓库或宿主机全局配置。

### Tests

- **完整验证**：144 项 Rust 单元测试、CLI 冒烟测试、Clippy 零警告、21 项前端测试、类型检查、Lint、生产构建与 Docker 健康检查全部通过。
- **真实规模验证**：381 个仓库全量 Fetch 从 188 秒、24 个失败优化至 24.7 秒、381 个全部成功。
...
- **Docker 生产部署**：新增前后端多阶段镜像、Compose 编排、Nginx `dist` 托管与 SSE 反代、非 root 运行、只读根文件系统、健康检查、日志轮转、持久卷及受控仓库/SSH 挂载。
...
- **端口规范化**：后端默认端口调整为 `8615`，前端开发及 Docker 访问端口调整为 `9215`，并保留后端默认仅监听回环地址的安全边界。
...
- **两步批次单次消费**：第二步一经启动即消费当前 Fetch 批次；完成、失败或取消后流程统一返回第一步，禁止重复使用旧远程快照再次更新。
...
- **回归覆盖扩充**：Rust 单元测试增至 128 项并保留 CLI 冒烟测试；前端 21 项实体订阅、协议契约、筛选条件、批次状态及自定义模态交互测试通过，并执行严格类型检查、Oxlint、Vitest 和生产构建预算校验。
