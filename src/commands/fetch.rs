...
pub async fn execute(
    ...
    // 批量任务存在失败时必须返回非零退出码，避免脚本把不完整结果当成成功。
    ensure_fetch_complete(&summary)?;

    Ok(())
}

fn ensure_fetch_complete(summary: &crate::fetcher::FetchSummary) -> Result<()> {
    if summary.failed > 0 {
        anyhow::bail!(
            "Fetch 未完整成功：{} 个成功，{} 个失败",
            summary.success,
...
mod tests {
    ...
    #[test]
    fn partial_fetch_summary_is_not_successful() {
        let mut summary = crate::fetcher::FetchSummary::new();
        summary.success = 2;
        summary.failed = 1;

        assert!(ensure_fetch_complete(&summary).is_err());
    }
}
