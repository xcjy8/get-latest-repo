...
impl Fetcher {
    ...
    #[cfg(test)]
    pub(crate) fn configured_proxy(&self) -> &ProxyConfig {
        &self.proxy
    }

    #[cfg(test)]
    pub(crate) fn configured_timeout_secs(&self) -> u64 {
        self.timeout_secs
    }
    ...
    /// 注册结构化进度观察器；Web 控制台使用它推送实时进度，CLI 可保持为空。
    ...
    /// 注册取消检查器；已排队任务会在取得并发许可后、执行 Git 前安全退出。
    ...
    async fn prescan_security_batch(
        &self,
        repos: &[Repository],
        ...
        let mut scan_failed_paths = HashSet::new();
            ...
            let future = tokio::spawn(async move {
                ...
                .await
                {
                    ...
                    _ => {
                        ...
                        return Some((path_str, "安全扫描失败，已按高风险处理".to_string(), true));
                    }
                };

                if !scan_result.0 {
                    Some((path_str, scan_result.2, false))
                } else {
                    if scan_result.1 {
                        eprintln!("{}", scan_result.2);
                    }
                    None
                }
            });
        ...
        while let Some(result) = futures.next().await {
            if let Ok(Some((path, report, scan_failed))) = result {
                if scan_failed {
                    scan_failed_paths.insert(path.clone());
                }
                risky_reports.insert(path, report);
            }
        }

        (risky_reports, scan_failed_paths)
    }
    ...
    pub async fn fetch_all_detailed(
        ...
        let rejected_paths = if self.security_scan {
            let (risky_reports, scan_failed_paths) = self.prescan_security_batch(repos).await;
            if self.auto_allow_high_risk {
                // Web 明确确认后自动放行真实高风险项；扫描失败项继续 fail-closed。
                scan_failed_paths
            } else if self.auto_skip_high_risk {
                // 自动跳过高风险仓库，无需交互确认
        ...
        for repo in repos {
            ...
            let future = tokio::spawn(async move {
                    ...
                    .is_some_and(|checker| checker())
                {
                    ...
                    return (original_repo, repo, result, None, false);
                }
                ...
                for attempt in 0..=MAX_RETRIES {
                    ...
                    let cancellation_for_fetch = cancellation_checker.clone();
                    ...
                    fetch_status = match timeout(
                        ...
                        tokio::task::spawn_blocking(move || {
                            ...
                            git_ops.fetch_detailed_with_cancellation(
                                &path,
                                attempt_timeout.as_secs(),
                                cancellation_for_fetch.as_deref(),
                            )
                        }),
                    )
                    .await
                ...
                let (current_repo, result, moved_repo_name, restored_from_needauth) =
                    if fetch_status.should_move_to_needauth() && move_to_needauth {
                        ...
                        // 目录移动属于破坏性操作，必须等待阻塞任务完成；异步超时会
                        // 丢弃 JoinHandle，却无法终止底层文件系统写入。
                        let move_result = match tokio::task::spawn_blocking(move || {
                        ...
                        .await
                        {
                            Ok(Ok(path)) => Ok(path),
                            Ok(Err(e)) => Err(e),
                            Err(_) => Err(anyhow::anyhow!("移动任务 panic")),
                        };
                        ...
                        && Self::path_is_in_needauth(&original_path, &needauth_root)
                    {
                        ...
                        .await
                        {
                            Ok(Ok(path)) => Ok(path),
                            Ok(Err(e)) => Err(e),
                            Err(_) => Err(anyhow::anyhow!("恢复任务 panic")),
                        };
                    ...
                    match join_result {
                        Ok((original_repo, current_repo, result_model, moved_name, restored)) => {
                            ...
                            if let Some(observer) = self.progress_observer.as_ref() {
                                observer(results.len(), repos.len());
                            }
                        }
                        Err(e) => {
    ...
    /// 同名目标无论来源是否一致都必须保留，新的隔离副本使用数字后缀。
    /// remote URL 只能证明配置相同，不能证明分支、stash 与工作区内容完全一致。
    ///
    ...
    fn move_repo_to_needauth(
        ...
        // 唯一名称生成后仍需再次检查，防止极短竞态窗口内出现同名目录。
        if final_to.exists() {
            return Err(anyhow::anyhow!(
                "目标目录 '{}' 已被占用，已停止移动以保护现有数据",
                final_to.display()
            ));
        }

        Self::move_or_copy_dir(Path::new(from), &final_to)
            .with_context(|| format!("无法将仓库移动到安全隔离目录 '{}'", final_to.display()))?;
    ...
    ///
    /// # Safety rules
    /// - If target path exists → stop and preserve both copies
    ...
    fn move_repo_from_needauth(
        ...
        if to.exists() {
            // remote URL 相同不代表 refs、stash 或工作区相同，自动删除任一副本都会丢数据。
            return Err(anyhow::anyhow!(
                "目标路径 '{}' 已存在，已保留原位置与 needauth 两份数据，请人工确认后处理",
                to.display()
            ));
        }
    ...
    fn scan_repository(
        path: &std::path::Path,
        _name: &str,
    ) -> Result<(bool, bool, String), anyhow::Error> {
        ...
        Ok((result.is_safe, result.has_risks(), report))
    }
...
mod tests {
    ...
    #[test]
    fn fetch_high_risk_automatic_policies_are_mutually_exclusive() {
        let allow = Fetcher::new(5, 30).with_auto_allow_high_risk(true);
    ...
    #[test]
    fn move_repo_preserves_existing_target_and_uses_unique_name() {
        let tmp = TempDir::new().unwrap();
        ...
        let moved_path = Fetcher::move_repo_to_needauth(
            source_dir.to_str().unwrap(),
            &target,
            target.parent().unwrap_or(tmp.path()),
            None,
        ...
        assert_eq!(moved_path.file_name().unwrap(), "repo-2");
        assert!(moved_path.join(".git").exists());
    }

    #[test]
    fn restore_preserves_both_copies_when_original_path_exists() {
        let tmp = TempDir::new().unwrap();
        let original = tmp.path().join("repos").join("repo");
        let isolated = tmp.path().join("needauth").join("repo");
        fs::create_dir_all(&original).unwrap();
        fs::create_dir_all(&isolated).unwrap();
        init_git_repo(&original);
        init_git_repo(&isolated);
        fs::write(isolated.join("独有数据.txt"), "必须保留").unwrap();

        let result = Fetcher::move_repo_from_needauth(
        ...
        assert_eq!(
            fs::read_to_string(isolated.join("独有数据.txt")).unwrap(),
            "必须保留"
        );
    }
}
