...
#[command(about = "快速、安全、实时的本地 Git 仓库批量管理工具")]
#[command(version)]
pub struct Cli {
    #[command(subcommand)]
...
mod tests {
    ...
    #[test]
    fn scan_output_and_out_options_are_unambiguous() {
        // Clap 的结构检查会直接发现重复短参数，防止 Debug 二进制在运行时 panic。
        ...
        .expect("scan 输出格式与文件路径应能同时解析");

        match cli.command {
            Commands::Scan { output, out, .. } => {
                assert!(matches!(output, OutputFormat::Markdown));
                assert_eq!(out, Some(PathBuf::from("report.md")));
            }
            _ => panic!("应解析为 scan 命令"),
        }
    }
}
